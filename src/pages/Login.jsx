...
export default function Login() {
	...
	const renderError = (show, message) => (show && message ? <p className="mt-1 text-xs text-red-600">{message}</p> : null);

	return (
		<Layout isLoggedIn={false}>
			<div className="flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
				<div className="max-w-lg w-full bg-white p-8 rounded-2xl shadow-lg border border-gray-100">
					{globalError && <div className="mb-4 px-4 py-3 rounded-lg bg-red-50 border border-red-200 text-sm text-red-700">{globalError}</div>}
					...
					{role === "mahasiswa" && (
						<>
							...
							<form className="space-y-5" onSubmit={submitMahasiswa} noValidate>
								...
								<div>
									<label htmlFor="mhs-password" className="block text-sm font-medium text-gray-700 mb-1">
										Kata Sandi (Untuk Pendaftaran & Login)
									</label>
									<div className="relative">
										...
										/>
										<button type="button" onClick={() => setShowPassword((v) => !v)} className="absolute inset-y-0 right-0 px-3 text-xs font-semibold text-gray-500 hover:text-kampus-blue">
								...
								<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
									...
									<div>
										...
										<select id="prodi" name="prodi" value={mahasiswa.prodi} onChange={updateMahasiswa("prodi")} onBlur={markTouched("prodi")} required className={selectClass(!!errors.prodi)}>
											<option value="" disabled>
												Pilih Prodi
											</option>
